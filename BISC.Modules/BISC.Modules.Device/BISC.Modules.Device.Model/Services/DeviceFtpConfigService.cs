...
namespace BISC.Modules.Device.Model.Services
{
    public class DeviceFtpConfigService : IDeviceFtpConfigService
    {
        ...
        public async Task<OperationResult> SaveDeviceFtpConfig(string ip, IDeviceFtpConfig deviceFtpConfigToSave)
        {
            try
            {
                var configAsStringRes = await _deviceFileWritingServices.ReadFileStringFromDevice(ip, "1:/CFG", "CONFIG.CFG");
                if (configAsStringRes.IsSucceed)
                {
                    var configString = configAsStringRes.Item;
                    // Replacing config values of configuration file
                    configString = 
                        Regex.Replace(configString, @"KEY=(.*)", $"KEY={deviceFtpConfigToSave.TechKey}");

                    var res = await _deviceFileWritingServices.WriteFileStringInDevice(ip, new List<string>() { configString },
                        new List<string>() { "CONFIG.CFG" });
                    if (!res.IsSucceed)
                    {
            ...
            await _deviceFileWritingServices.ResetDevice(ip);
            return OperationResult.SucceedResult;
        }

        #endregion

        private void Write(IDeviceFtpConfig config, TextWriter streamTextWriter)
        {
            //streamTextWriter.WriteLine($"#MAC={config.MacAddress}");
            streamTextWriter.WriteLine($"KEY={config.TechKey}");
